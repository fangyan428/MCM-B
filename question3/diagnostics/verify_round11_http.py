...
def main():
    root=Path('question3/results/round11_self_http');root.mkdir(exist_ok=False);rows=[]
    for name in ['P']:
        env=Simulator(random_case(1));server=make_server(env)
        thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start()
        out=root/name;config=Path('question3/configs/round11_dev',name+'.json')
        try:
            ...
            count=sum(e['type']=='certified_clear_skip' for e in es)
            assert count>0
            ...
            rows.append(dict(variant=name,kind='SELF_REAL_LOOPBACK_HTTP_NOT_OFFICIAL',complete=True,
                certified_clear_skips=count,virtual_time_s=result['metrics']['virtual_time_s']))
