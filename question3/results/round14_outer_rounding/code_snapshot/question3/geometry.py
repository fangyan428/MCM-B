...
def stations(radius=1500.,phase_rad=0.):
    return [(0.,0.)]+[(radius*math.cos(phase_rad+k*math.pi/3),radius*math.sin(phase_rad+k*math.pi/3)) for k in range(6)]


def station_cover_bound(radius=1500.):
    if not math.isfinite(radius) or radius<=0:raise ValueError('Invalid search ring radius')
    # Centre covers radius <=900. Remaining annulus: nearest ring angle <=30deg.
    return max(900.,*(math.sqrt(r*r+radius**2-2*r*radius*math.cos(math.pi/6)) for r in [900,1800]))
