"""Exercise FREE through the real CLI and local self HTTP; not official."""
import json
...
def main():
    root=Path('question3/results/round16_self_http');root.mkdir(exist_ok=False);rows=[]
    for name in ['FREE']:
        env=Simulator(random_case(0));server=make_server(env)
        thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start()
        out=root/name;config=Path('question3/configs/round16_dev',name+'.json')
        try:
            ...
            rows.append(dict(variant=name,kind='SELF_REAL_LOOPBACK_HTTP_NOT_OFFICIAL',complete=True,
                forced_scans=count,scheduled_tasks=len(tasks),virtual_time_s=result['metrics']['virtual_time_s']))
