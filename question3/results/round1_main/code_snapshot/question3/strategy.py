...
def run(client, config, event=lambda data:None):
    ...
    try:
        while True:
            ...
            station_id=min(remaining,key=lambda k:(math.dist(client.position,cover[k]),k)) if remaining else None
            jobs=[]
            for c in pending:
                q,sel=choose_second(*first[c],client.position,config)
                jobs.append((math.dist(client.position,q),c,q,sel))
            job=min(jobs,key=lambda x:(x[0],x[1])) if jobs else None
            if station_id is not None and (job is None or consecutive_localizations>=2 or math.dist(client.position,cover[station_id])<=job[0]):
                event(dict(type='schedule',task='scan',station_id=station_id,forced=consecutive_localizations>=2))
                scan(station_id);consecutive_localizations=0
            else:
                _,c,q,sel=job;event(dict(type='schedule',task='localize',channel=c))
