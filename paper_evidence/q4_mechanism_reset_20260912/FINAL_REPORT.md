...
结论：没有找到满足预设门槛的显著更好方案。锁定新候选 COVER_EXCHANGE_ELASTIC 在最终 120 个未见布局上平均总虚拟时间降低 **0.366%**，P95 降低 **0.394%**。建议保留原 25 站推荐，不为小幅收益增加复杂度。
...
|---|---:|---:|---:|---:|---:|---:|
|BASE25|960/960|0/0|6744.38|9497.47|11986.29|0.000%|
|HIST_ADAPT|960/960|0/0|6614.15|9417.57|12592.28|1.931%|
|COVER_EXCHANGE|960/960|0/0|6734.42|9497.47|11986.29|0.148%|
|COVER_ELASTIC|960/960|0/0|6740.98|9460.06|11928.43|0.050%|
|COVER_EXCHANGE_ELASTIC|960/960|0/0|6719.67|9460.06|11928.43|0.366%|

锁定候选均值收益的布局分块、家族分层配对 bootstrap 95%区间为 **[-0.183%, 0.921%]**（10000次）。706局更快、254局更慢、0局相同。该区间描述本自建生成器的布局采样变化，不能代表未知官方分布。
...
|---|---:|---:|---:|
|detection_s|1624.92|1631.78|+6.86|
|laser_s|26.67|26.67|+0.00|
|movement_s|4687.34|4650.34|-37.00|
|optical_s|103.42|107.39|+3.97|
|switch_s|302.04|303.49|+1.45|

阶段分解（秒/局）：

|阶段|原推荐|候选|
|---|---:|---:|
|certified_clear|465.58|448.76|
|control|0.00|0.00|
|localize|608.42|612.24|
|optical_fallback|916.59|948.70|
|search|4753.79|4709.97|

动作组件与阶段是同一总耗时的两种分组，不能相加重复计费。

## 十轮结果与放弃原因

|轮|机制|开发均值收益|决定|
|---|---|---:|---|
|01|ACTUAL_TASK|-0.800%|未达门槛|
|02|BACKBONE|-4.467%|未达门槛|
|03|WINDOW|-17.472%|未达门槛|
|04|COVER_SEARCH_FIRST|-12.983%|未达门槛|
|05|COVER_SECTOR|-13.954%|未达门槛|
|06|COVER_EXCHANGE|0.256%|未达门槛|
|07|COVER_ELASTIC|2.150%|未达门槛|
|08|CELL_OPTICAL|-0.849%|未达门槛|
|09|CLEAR_DISCOVERY|-6.642%|未达门槛|
|10|NEGATIVE_REGION|0.001%|未达门槛|

动作入口/出口建模未弥补预测误差；固定骨架、两站批处理、全巡视后清除及分区承诺均失去了原在线调度的重要适应性。动态替站触发有限；移动测站获得温和收益，组合仍低于门槛。减少光学单元并不保证早命中更早；清除后额外发现消耗了更多检测和切频；方向阴性位置收缩安全但触发/边际效应很小。每轮详细组件差、最差配对案例、事件频次、配置和原始日志见相应 round*/REVIEW.md 和 development/。

## 最差案例和可靠性

最差相对案例 `uniform_4c920b65f2c8599c__positive__bounded`：原推荐 4228.69 秒，候选 5653.40 秒，变慢 1424.70 秒（33.69%）。两者均完整清除。候选最大绝对耗时案例为 `outward_6ae342c484d8ba19__positive__bounded`。
...
本机每局策略计算均值从 0.0582 秒升至 0.1643 秒，P95从 0.0852 秒升至 0.2971 秒。两者均远低于本机现实期限，但候选引入了更多在线优化计算；这些不是官方网络/Windows实测时间。
