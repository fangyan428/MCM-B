...
def choose_second(first,bearing,current,config):
    if config.get('second_range_prior','off')=='domain':
        from .domain_range import range_upper,choose
        upper=range_upper(first,bearing,config['error_deg'])
        if upper<1500:return choose(first,bearing,current,config,design_catalog(config['error_deg'],config['min_angle_deg']),upper)
