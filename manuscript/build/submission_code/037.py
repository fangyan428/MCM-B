...
class DirectionalMechanismTests(unittest.TestCase):
    ...
    def test_directional_shadow_random_physics_and_independent_audit(self):
        rng = np.random.default_rng(749013)
        for index in range(400):
            truth = rng.uniform(-1100., 1100., 2)
            angle = rng.uniform(0., 360.)
            theta = math.radians(angle)
            rotation = np.array([[math.cos(theta), -math.sin(theta)], [math.sin(theta), math.cos(theta)]])
            extents = rng.uniform(1., 75., 2)
            poly = truth+np.array([[-1, -1], [1, -1], [1, 1], [-1, 1]])*extents
            n = truth+np.array([-rng.uniform(10., 650.), rng.uniform(-150., 150.)]) @ rotation.T
            positives = truth+np.array([[rng.uniform(10., 300.), rng.uniform(100., 500.)],
                                       [rng.uniform(10., 300.), -rng.uniform(100., 500.)]]) @ rotation.T
            coefficients = rng.uniform(.01, 2., 2)
            query = n+coefficients @ (n-positives)
            proof = shadow_certificate(poly, positives, [n], query)
            self.assertIsNotNone(proof)
            model = source(x=float(truth[0]), y=float(truth[1]), direction=angle, radius=rng.uniform(1000., 1500.))
            case, client, _, _, _ = setup([model])
            events = []
            for p in positives:
                response = client.measure(p, 1)
                self.assertNotEqual(response['measure_result'], 'no_signal')
                events.append(dict(event='observe', channel=1, point=p.tolist(), kind=response['measure_result']))
            self.assertEqual(client.measure(n, 1)['measure_result'], 'no_signal')
            events.append(dict(event='observe', channel=1, point=n.tolist(), kind='no_signal'))
            self.assertEqual(client.measure(query, 1)['measure_result'], 'no_signal')
            events.append(dict(event='directional_shadow_skip', channel=1, point=query.tolist(),
                               vertices=poly.tolist(), certificate=proof))
            self.assertEqual(audit_regions(case, events)['checked_directional_shadows'], 1)

    def test_directional_shadow_rejects_range_cone_boundary_and_degeneracy(self):
        poly = np.array([[-10., -10.], [10., -10.], [10., 10.], [-10., 10.]])
        positive = np.array([[100., 100.], [100., -100.]])
        n = np.array([-100., 0.])
        self.assertIsNotNone(shadow_certificate(poly, positive, [n], (-300., 0.)))
        for query in ((0., 0.), (100., 0.), (-100., 200.), n, n+(n-positive[0])):
            self.assertIsNone(shadow_certificate(poly, positive, [n], query))
        self.assertIsNone(shadow_certificate(poly, positive, [(-1000., 0.)], (-2000., 0.)))
        self.assertIsNone(shadow_certificate(poly, positive, [(-989.999999, 0.)], (-2000., 0.)))
        for flat in ([[100., 0.], [200., 0.]], [[100., 0.], [200., 1e-10]], [[100., 100.], [100., 100.]]):
            self.assertIsNone(shadow_certificate(poly, flat, [n], (-300., 0.)))
        self.assertIsNone(shadow_certificate(poly, positive[:1], [n], (-300., 0.)))
        self.assertIsNone(shadow_certificate(poly, positive, [], (-300., 0.)))

    def test_independent_shadow_audit_rejects_forward_skip_and_fake_witness(self):
        poly = [[-10., -10.], [10., -10.], [10., 10.], [-10., 10.]]
        positive = [[100., 100.], [100., -100.]]
        n, query = [-100., 0.], [-300., 0.]
        case = dict(sources=[source(x=0., direction=0.)])
        history = [dict(event='observe', channel=1, point=p, kind='direction') for p in positive]
        history.append(dict(event='observe', channel=1, point=n, kind='no_signal'))
        event = dict(event='directional_shadow_skip', channel=1, vertices=poly, point=query,
                     certificate=shadow_certificate(poly, positive, [n], query))
        self.assertEqual(audit_regions(case, history+[event])['checked_directional_shadows'], 1)
        for mutation in ({'point': [100., 0.]}, {'point': [-301., 0.]}):
            changed = copy.deepcopy(event); changed.update(mutation)
            with self.assertRaises(AssertionError):
                audit_regions(case, history+[changed])
        with self.assertRaises(AssertionError):
            audit_regions(case, [event])
        omni = dict(sources=[source(x=0., direction=None)])
        with self.assertRaises(AssertionError):
            audit_regions(omni, history+[event])
