...
@lru_cache(maxsize=1)
def radial_mesh():
    # 12 inner + 12 outer stations. Outer polygon apothem >1800,
    # all Delaunay edges <1000. Same halfplane proof, shorter outside travel.
    points=np.array([(0.,0.)]+[(r*math.cos(2*math.pi*k/12+phase),r*math.sin(2*math.pi*k/12+phase))
        for r,phase in ((950.,0.),(1870.,math.pi/12)) for k in range(12)])
    triangles=Delaunay(points).simplices
    assert max(np.linalg.norm(points[t][:,None]-points[t][None,:],axis=2).max() for t in triangles)<1000
    return points,triangles

def cover_file():
    return Path(__file__).with_name('certificates')/'compact22.json'

@lru_cache(maxsize=1)
def compact_mesh():
    data=json.loads(cover_file().read_text())
    if data['failed_cells']:raise ValueError('Uncertified compact cover')
    return np.array(data['stations']),np.empty((0,3),dtype=int)

def cover_hash():
    return hashlib.sha256(cover_file().read_bytes()).hexdigest()
