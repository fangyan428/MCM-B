...
def run(client, config, event=lambda data:None):
    ...
    range_prior=config.get('second_range_prior','off')
    if range_prior not in ('off','domain'):raise ValueError('Unknown second_range_prior')
    if range_prior!='off':
        if opportunity_timing!='after_schedule' or not config.get('modules',{}).get('A',False):
            raise ValueError('Domain second-range prior requires A and after-schedule timing')
        planning_config={**planning_config,'second_range_prior':'off'}
    shared_range_prior=config.get('shared_range_prior','off')
    if shared_range_prior not in ('off','domain'):raise ValueError('Unknown shared_range_prior')
    if shared_range_prior!='off' and (not config.get('modules',{}).get('S',False) or config.get('shared_selection','lowest_id')!='lowest_id'):
        raise ValueError('Shared domain range requires S and lowest-id selection')
    ...
    scan_order=config.get('scan_order','nearest')
    if scan_order not in ('nearest','remaining_open_path'):raise ValueError('Unknown scan_order')
    if scan_order!='nearest' and (not joint or order or cache_route!='nearest' or config.get('routing',{}).get('mode','nearest')!='nearest' or phase_mode!='fixed'):
        raise ValueError('Scan path ordering requires B nearest routing, fixed stations, no O/cache insertion')
    ...
    def localize(channel,q,selection):
        if channel in cached:
            ...
            points=orient(channel,points)
            batch_limit=current_shared_limit if selection['mode'] in ('opportunistic_current','domain_current') else shared_limit
    ...
    try:
        while True:
            ...
            # B: compare task starts; optional gates govern scan interleaving.
            jobs=[]
