...
def source_hashes():
    paths=list(Path('question4').glob('*.py'))+list(Path('question4/certificates').glob('*.json'))+[Path('question3/interface.py')]
...
def main():
    ...
    p.add_argument('--error-modes',default='spatial_hash',help='Comma-separated spatial error laws, crossed across families')
    ...
    lock['error_modes']=args.error_modes.split(',')
    if args.lock:
    ...
    if not args.fixtures:
        for i,case in enumerate(cases):
            case['error_mode']=lock['error_modes'][(i//6)%len(lock['error_modes'])]
            case['id']=case['id'].rsplit('_',2)[0]+'_'+case['error_mode'] if case['error_mode']!='spatial_hash' else case['id']
    ...
    for case in cases:
        ...
        write(out/'summary.json',dict(stage=args.stage,rows=rows,platform=platform.platform(),
            environment=dict(python=sys.version,numpy=np.__version__,scipy=scipy.__version__),elapsed_s=time.perf_counter()-started))
