...
def check_pair(base,new):
    ...
    assert [(e['point'][0],e['point'][1],e['channel']) for e in omitted]==[(e[1],e[2],e[3]) for e in deleted]
    return len(deleted)
...
def review(suffixes):
    ...
    for suffix in suffixes:
        path=root/('round11_'+suffix);s=json.loads((path/'summary.json').read_text());rows=s['cases']
        ...
        skipped=0;pairs=[]
        for i in ids:
            a,b=lookup['REF',i],lookup['P',i]
            n=check_pair(path/'REF'/i,path/'P'/i);skipped+=n
        ...
        report['batches'][suffix]=dict(runs=len(rows),complete=len(rows),failed=0,timeouts=0,false_complete=0,
            ...
            passed_gate=stat['P']['mean_virtual_s']<stat['REF']['mean_virtual_s'] and stat['P']['p95_virtual_s']<=stat['REF']['p95_virtual_s'],
    ...
    for first,second in zip(suffixes[::2],suffixes[1::2]):
        a=json.loads((root/('round11_'+first)/'summary.json').read_text())
        b=json.loads((root/('round11_'+second)/'summary.json').read_text())
...
def main():
    ...
    for rounding in ['main','rounding']:
        ...
        for batch in ['dev','adverse','holdout','outer']:
            data=json.loads(Path(f'question3/results/round9_{batch}_{rounding}/summary.json').read_text())
    ...
    if not args.development_only:
        lock=json.loads(Path('question3/results/round11_lock.json').read_text())
        for name in names:
            summary=json.loads(Path(f'question3/results/round11_{name}/summary.json').read_text())
            for k in ['code_sha256','configs']:assert summary[k]==lock[k]
    report['legacy_exact_comparisons']=legacy
    report.update(fresh_validation_runs=0 if args.development_only else 1120,fresh_layouts=0 if args.development_only else 70)
    out=Path('question3/results/round11_development_evidence.json' if args.development_only else 'question3/results/round11_review_evidence.json')
