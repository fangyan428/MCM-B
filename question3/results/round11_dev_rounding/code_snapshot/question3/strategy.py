...
from .clear_skip import constraints,failure_certificate
...
def run(client, config, event=lambda data:None):
    ...
    if clear_skip not in ('off','separating_halfplane'):raise ValueError('Unknown clear_skip')
    ...
    def localize(channel,q,selection):
        ...
        for i in range(len(points)):
            ...
            if clear_skip!='off':
                proof=failure_certificate(p,skip_planes.get(channel))
                if proof:
