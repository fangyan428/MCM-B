...
def main():
    root=Path.cwd();archive=root/'question3/releases/q3_omni_round16.zip'
...
lock=json.loads(Path('question3/results/round16_lock.json').read_text())
assert c==lock['configs']['FREE']
assert all(hashlib.sha256(Path(p).read_bytes()).hexdigest()==h for p,h in lock['code_sha256'].items())
rows=[]
fresh=json.loads(Path('question3/results/round16_holdout_cases.json').read_text())
worst=[c for c in fresh if c['id'] in ('holdout_210032_positive','holdout_210044_smooth')]
assert len(worst)==2
for case in [random_case(0,'negative'),random_case(1,'smooth'),late_source_case(),clustered_case()]+worst:
 ...
 rows.append(dict(tasks=len(tasks),case=case['id'],virtual_s=r['metrics']['virtual_time_s'],radius_skips=sum(e['type']=='certified_clear_skip' and 'radius_prior' in e for e in es),certificate=r['stopping_certificate']['method'],audit_actions=audit_folder(folder)))
assert rows[2]['certificate']=='seven_station_channel_cover'
assert rows[3]['certificate']=='source_count_upper_bound'
print(json.dumps(rows))
'''
    with tempfile.TemporaryDirectory(prefix='q3-round16-release-') as temp:
        with ZipFile(archive) as z:
            assert not [i for i in z.infolist() if not i.file_size and 'round16' in i.filename]
            z.extractall(temp)
    ...
    (root/'question3/results/round16_release_package_smoke.json').write_text(json.dumps(data,indent=2)+'\n')
