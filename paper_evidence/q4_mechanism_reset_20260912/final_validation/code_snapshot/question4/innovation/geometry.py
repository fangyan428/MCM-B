...
def optical_order(points, current, mode='original'):
    """Permute an unchanged finite covering grid; never discard a point."""
    n = len(points)
    if mode == 'original':
        return list(range(n))
    if mode != 'nearest_branch':
        raise ValueError('Unknown optical order')
    k = min(range(n), key=lambda i: (math.dist(current, points[i]), i))
    orders = [list(range(k, -1, -1))+list(range(k+1, n)),
              list(range(k, n))+list(range(k-1, -1, -1))]
    def distance(order):
        return math.dist(current, points[order[0]])+sum(
            math.dist(points[a], points[b]) for a, b in zip(order, order[1:]))
    return min(orders, key=distance)
