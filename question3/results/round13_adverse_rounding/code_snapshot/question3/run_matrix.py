...
def main():
    ...
    parser.add_argument('--config-dir',default='question3/configs/round1')
    parser.add_argument('--fixtures')
    parser.add_argument('--stage',default='SELF_ROUND1_DEVELOPMENT_NOT_HOLDOUT')
    parser.add_argument('--lock')
    args=parser.parse_args();out=Path(args.output);out.mkdir(parents=True,exist_ok=False)
    names=VARIANTS if args.config_dir=='question3/configs/round1' else sorted(p.stem for p in Path(args.config_dir).glob('*.json'))
    configs={n:json.loads((Path(args.config_dir)/(n+'.json')).read_text()) for n in names}
    if args.lock:
        lock=json.loads(Path(args.lock).read_text())
        for name,digest in lock['code_sha256'].items():
            if hashlib.sha256(Path(name).read_bytes()).hexdigest()!=digest:raise RuntimeError('Locked source changed: '+name)
        if configs!=lock['configs']:raise RuntimeError('Locked configurations changed')
    if args.fixtures and not args.lock:raise RuntimeError('External validation fixtures require a pre-run configuration lock')
    fixtures=json.loads(Path(args.fixtures).read_text()) if args.fixtures else list(development_cases());rows=[]
    ...
    for i,case in enumerate(fixtures):
        ...
        for name in order:
            ...
            row=dict(variant=name,case_id=case['id'],kind=args.stage,rounding=args.rounding,
        ...
        print(f'{i+1}/{len(fixtures)} {case["id"]}: '+', '.join(f'{r["variant"]}={r["metrics"]["virtual_time_s"]:.0f}' for r in rows[-len(names):]),flush=True)
        summary=dict(kind=args.stage,rounding=args.rounding,cases=rows,
                     ...
                     official_rehearsals_run=0,formal_tests_run=0,unseen_validation_cases_run=(i+1 if args.fixtures else 0),config_dir=args.config_dir)
