"""Exercise J and Z through the real CLI and local self HTTP; not official."""
import json
...
def main():
    root=Path('question3/results/round9_self_http');root.mkdir(exist_ok=False);rows=[]
    for name in ['JL','ZL']:
        env=Simulator(random_case(0));server=make_server(env)
        thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start()
        out=root/name;config=Path('question3/configs/round9_dev',name+'.json')
        try:
            ...
            count=sum(e['type']=='localization' and e['selection']['mode']=='opportunistic_current' for e in es)
            assert count>0
            ...
            rows.append(dict(variant=name,kind='SELF_REAL_LOOPBACK_HTTP_NOT_OFFICIAL',complete=True,
                actual_current_measurements=count,virtual_time_s=result['metrics']['virtual_time_s']))
