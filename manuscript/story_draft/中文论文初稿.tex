...
无线电干扰源的定位通常包括初步监测、区域性排查和精准定位三个阶段。前两个阶段可借助固定监测站、移动监测车或无人机确定目标分布区域，精准定位与清除仍需技术人员携带设备进入区域内实地排查。为在危险环境中代替人工开展作业，某公司拟研制搭载无线电测向机、光学探测仪和激光清除装置的机器狗，自动完成干扰源的搜索、定位与清除，并尽量缩短任务时间\textsuperscript{\cite{problem}}。

目标区域为半径1800 m的圆域，各干扰源占据1---20中的不同频道，位置及有效接收半径未知，接收半径在1000---1500 m之间。机器狗需停止移动后逐频道检测，正常读数给出干扰源的示向度，其测向误差在±1°范围内；同一地点对同一源重复检测的误差固定。通过不同检测点的方位信息可开展交会定位，进入距源20 m范围后，可由光学探测仪精确定位并启动清除装置\textsuperscript{\cite{problem,simulator,interface}}。
...
图1概括四问的递进关系：第一问以测向取交建立位置区域与覆盖判据；第二问据此约束第二测点的接收条件和交会几何；第三问将局部定位嵌入多源搜索与滚动调度；第四问针对定向失联重建发现与清除保证。下方闭环表示Q3与Q4的在线执行，任务完成后依据观测和清除反馈更新记录并重排。
...
\caption{整体技术路线。上方按Q1至Q4展示方法递进，下方为Q3与Q4的在线执行闭环。几何小图为非定量示意，不是实测轨迹；箭头表示方法衔接或执行反馈。}
\end{figure}

\clearpage
\section{3 模型假设}\label{ux6a21ux578bux5047ux8bbe}

沿用题面及接口说明中的设备参数和操作规则，本文采用以下模型前提\textsuperscript{\cite{problem,simulator,interface}}。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth,trim={0 175bp 0 65bp},clip]{figures/fig04_q2_region_optimized.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth,trim={0 90bp 0 45bp},clip]{figures/fig17_shrink_optimized.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.76\linewidth,trim={0 110bp 0 0},clip]{figures/fig07_q3_cover_optimized.png}
...
\FloatBarrier
\subsection{5.4 问题四：混合源的搜索定位与清除}\label{sec-5-4}
...
\subsubsection{5.4.1 混合源的位置区域模型}\label{sec-5-4-1}

记已发现频道集合为\(\mathcal D_t\)，已成功清除集合为\(\mathcal C_t\subseteq\mathcal D_t\)，未完成搜索站编号集合为\(\mathcal S_t\)。其中\(\mathcal D_t\)包含已清除频道；正常方位或近场反馈均可确认发现一个源，成功清除后才将其加入\(\mathcal C_t\)。每个已知未清频道\(c\)维护位置区域\(P_c\)、首测示向度及已有有效方位记录。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.76\linewidth]{figures/fig08_no_signal.pdf}
\caption{定向源的无信号反例。测站位于源的发射背面，即使距离小于1000 m也无法接收。无信号时保留位置区域，光学清除仍只受20 m距离条件限制。}
\end{figure}

\FloatBarrier
\subsubsection{5.4.2 搜索站布设与连续覆盖}\label{sec-5-4-2}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\linewidth]{figures/fig09_q4_cover.pdf}
\caption{25站双环搜索构造。中心站与内、外环分别位于0、950和1870 m处，两环错开15°，连接形成36个三角形。最长边小于1000 m，外环正多边形包围目标圆域，从而对任意源位置和发射方向提供发现保证。}
\end{figure}

图13将发现保证分为局部与全局两部分：局部三角形内的源至少有一个近距顶点位于发射正面或边界；全局外环多边形包围整个源域。外环内切半径比源域半径多6.28 m，该余量按边到圆心的垂距计算。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.80\linewidth,trim={0 25bp 0 135bp},clip]{figures/fig20_q4_coverage_certificate.png}
...
\FloatBarrier
\subsubsection{5.4.3 交会定位与光学清除}\label{sec-5-4-3}
...
按蛇形顺序访问格心，并从距当前位置较近的一端开始；每次尝试后检查清除反馈，成功即结束该源服务。任意格内一点至格心的距离满足

\[
\frac12\sqrt{d_u^2+d_v^2}\le14\sqrt2\approx19.80<20\ \mathrm m.
\]

由于区域始终保留真源，含真源的格必被覆盖；光学清除不受发射方向限制，因此后续无线电失联也不妨碍有限清除。初始矩形长不超过1500 m、宽不超过\(2\cdot1500\sin1.005^\circ\approx52.62\) m，至多需要\(\lceil1500/28\rceil\lceil52.62/28\rceil=108\)个格心，后续正向观测只会缩小范围。这是初始访问上界，实际可能提前成功；所有失败尝试的移动及3 s光学费用均计入总时间。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig10_optical.pdf}
\caption{有限光学网格。每格两边不超过28 m，因此格心的20 m清除圆覆盖整个格。图示六格局部区域及蛇形访问顺序，实际格数由当前位置区域决定，清除成功后停止访问。}
\end{figure}

\FloatBarrier
\subsubsection{5.4.4 搜索与清除的滚动调度}\label{sec-5-4-4}
...
\FloatBarrier
\subsubsection{5.4.5 完整算法与求解结果}\label{sec-5-4-5}
...
\tightlist
\item
  \textbf{初始化。} 从原点进入，建立25站搜索任务、空的发现与清除集合、方位及无信号记录。
\item
  \textbf{检查完成条件。} 条件成立时退出并核验反馈；否则建立任务集合。若已发现16个不同频道，则在本轮规划中取消剩余未知搜索，继续服务已知未清源。
\item
  \textbf{选择任务。} 按上一节的开放路径规则排序，选择首项完整任务。
\item
  \textbf{执行与更新。} 搜索任务完成本站必要频道扫描：正常方位用于建立或更新区域，近场反馈直接清除，无信号记录于对应频道。源服务任务依次检查直接清除、必要的两侧补测与光学网格。成功清除后更新\(\mathcal C_t\)；整站扫描结束后更新\(\mathcal S_t\)，再返回步骤2。数量达到16时也在整站结束后的下一轮规划中取消搜索。
\item
  \textbf{异常处理。} 区域为空、保证清除失败、光学网格耗尽仍未成功，或接口执行异常、超时，均报告未完成；不能以当前任务集合为空替代停止核验。
\end{enumerate}

连续覆盖保证未知源能够被发现，位置区域保留真实源，有限光学覆盖保证已知源能够清除。每项外层任务完成一个搜索站或清除一个源，至多有25项搜索任务与16项源服务任务；路线调整和单源光学访问也均有限。由此在题面模型、合法反馈和准确几何运算条件下建立有限完成依据，实际程序仍需通过数值检查并满足现实执行时限。
...
算法输出实际任务访问序列、各频道清除记录、式（2）的时间分解及停止依据。25站布局、重心排序与观测筛选共同组成第四问推荐方案，其自建配对实验和其他布局、动态测站的比较见6.3节，正式测试见6.4节；不能将完整方案的降时比例归为站数变化的单独贡献。各问模块的输入、输出与适用条件汇总于附录B.3。
...
本章主要性能结果与统计图均由已冻结的逐例记录计算。Q2是自建几何算例；Q3和Q4为按题面及接口规则实现的自建完整流程实验。自建源布局用于比较策略，不被解释为官方案例的概率分布。几何复核、实验记录与正式测试材料的说明见附录A、C、D及随文支撑材料。后续新增的Q3调度开发比较单列于附录D.4，不并入原验证统计。
...
``布局''指固定的一组源位置、半径、频道以及适用时的方向；重复施加误差场不产生新的独立布局。采用自助法估计统计量的不确定性\textsuperscript{\cite{efron1979}}；本文以独立布局为重采样单位，重复有放回抽取布局块，并保留块内全部误差条件。同一物理布局上的算法使用相同误差条件，按配对方式比较。主要效应定义为
\(100(1-\overline T_{\rm candidate}/\overline T_{\rm base})\)，正值表示省时；图16、图20的纵轴使用相反符号的时间变化，正值表示变慢，图注逐一注明。完整清除数、均值、中位数、95\%分位数（P95，即本批约95\%运行耗时不超过的值）和本批最大值同时保留，避免均值掩盖失败和长尾。表中时间指标按该行全部运行等权计算，分位数采用线性插值；主要效应区间按布局重采样。箱线图以箱体表示第25\%至75\%分位数，中线表示中位数，须延伸到1.5倍四分位距范围内的最远观测，范围外样本仍保留。
...
图15先比较全部运行的时间分布：联合方案的中心位置和高耗时区间整体下降，但红色连线保留同一布局条件中的严重慢例，显示分布改进与逐例优势是两回事。
...
图16进一步展示收益的分布。每点对应一个布局的4种误差条件合并结果，全部120个布局均被保留。外缘、最小半径等场景多数省时，但聚集类仍存在明显正向离群点。它说明平均改进确实覆盖多类布局，同时也提醒我们：不同布局可提供的共享和提前停止机会并不相同。
...
图17将同一批结果分解为实际动作费用。联合方案每局平均少用383.25
...
图18用共同父方案分析几个组件。共同父方案除最终保留部分外，还包括切换任务时对目标原地再观测。删除这一额外步骤平均省25.00
...
图19直接比较31站初始基线与25站推荐的时间分布。箱体和高耗时观测整体下移，说明均值改进并非仅由少数极快案例造成；组件删减的均值比较则在下文单独说明。两批各96布局中，25站推荐平均时间分别为6640.86
s和6694.69
...
图20保留了所有布局的配对变化。组合的平均省时为0.366\%，95\%区间为{[}−0.183\%,
...
\clearpage
\subsection{B.3 算法模块的输入、输出与限制}\label{sec-B-3}
