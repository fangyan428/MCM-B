...
def main():
    ...
    if args.stage=='final_validation' and not args.lock:
        raise RuntimeError('Final validation requires a pre-generation source/config lock')
    if args.lock:
        lock=json.loads(Path(args.lock).read_text())
        for file,digest in lock['code_sha256'].items():
            if hashlib.sha256((ROOT/file).read_bytes()).hexdigest()!=digest:
                raise RuntimeError('Locked experimental source changed: '+file)
        for file,digest in lock['baseline_sha256'].items():
            if hashlib.sha256((ROOT/'frozen'/file).read_bytes()).hexdigest()!=digest:
                raise RuntimeError('Frozen baseline changed: '+file)
        for name,cfg in configs.items():
            if lock['configs'].get(name)!=cfg:raise RuntimeError('Locked config changed: '+name)
