...
def choose_shared(primary,point,first,cleared,cached,config,primary_points=None,decision_log=lambda e:None):
    ...
    for channel in sorted(first):
        ...
        if offset is None and config.get('shared_range_prior','off')=='domain':
            from .domain_range import range_upper,certified
            from .current_point import local_offset
            s,bearing=first[channel];upper=range_upper(s,bearing,config['error_deg'])
            q=local_offset(s,bearing,point)
            if upper<1500 and certified(q,upper,config['error_deg'],config['min_angle_deg']):
                return channel,dict(mode='domain_shared',offset=q,shared_with=primary,
                    source_range_upper_m=upper,range_prior='published_domain_1800',
