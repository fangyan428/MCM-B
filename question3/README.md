...
机制重探新增[独立创新候选与论文复现归档](innovation/README.md)：最终自建验证的整局均值从 3323.52 秒降至 2982.18 秒（10.27%），P95 下降，两方案均 480/480 完整清除；舍入压力收益 10.07%，最差配对案例仍慢 93.63%。[论文组织与原方法保留说明](innovation/PAPER_GUIDE.md)、[双方法复现与 HTTP 入口](innovation/REPRODUCE.md)。原策略、路线调度、推荐配置及 `question3.run_http` 不变；新方法独立入口为 `question3.run_innovation_http`，两者共用现有官方 HTTP 客户端。原冻结基线、十轮负结果、消融和最终验证数据均保留。

当前 Git pull 后要演练的新候选，直接使用 `python -m question3.run_innovation_http`；它固定读取 `innovation/best_candidate.json`，不要给它传 `--config`。完整步骤见[演练测试使用说明](演练测试使用说明.md)第 0 节和[创新方案复现说明](innovation/REPRODUCE.md)。
