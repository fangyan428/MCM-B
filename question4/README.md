...
2026-09-12 的十轮机制重新探索没有找到满足“平均总虚拟时间降低至少5%、完整清除成功率不降低、P95不恶化”的新方案，用户已决定保留本页的25站推荐。开发阶段锁定的动态替站与受覆盖约束测站移动组合，在最终120个新布局中仅平均省时0.366%，收益95%区间跨零；未替换 `configs/recommended.json`。

本次探索的代码、逐轮报告、汇总、场景和冻结快照见 [机制重新探索证据入口](../paper_evidence/q4_mechanism_reset_20260912/README.md)。更早的追加实验保留在 `innovation/`，其中候选也不代表当前推荐。云端保留可复现材料，逐案例原始日志和大压缩包留在本地，范围见 [云端同步说明](../CLOUD_SYNC.md)。

## 运行

完整的自建 HTTP 回归、Windows 官方“问题4演练测试”步骤、结果登记、日志保存和异常处理见：[演练测试使用说明](演练测试使用说明.md)。
