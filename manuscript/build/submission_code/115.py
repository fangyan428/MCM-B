...
from .geometry import initial,wedge,bbox,parallax_point,contains,worthwhile,enclosing
...
CONFIG=json.loads((ROOT/'best_candidate.json').read_text()) if (ROOT/'best_candidate.json').exists() else dict(baseline_config=BASE,mechanism='gated_tour_shared_parallax')
...
class Adversarial(unittest.TestCase):
        ...
        for _ in range(80):
            ...
            for g in list(poly)+[(a+b)/2 for a,b in zip(poly,np.roll(poly,-1,axis=0))]:
                ...
                for error in (-1.005,0,1.005):
                    ...
                    # Gate proves existence of a covering circle, not the radius of a newly rotated box.
                    self.assertLessEqual(enclosing(post)[1],max(20,old/2)+1e-5)
    ...
    def test_policy_has_no_oracle_or_file_access(self):
        ...
        for name in ('strategy.py','geometry.py','routing.py','coverage.py'):
