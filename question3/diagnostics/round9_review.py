"""Round9 fixed candidate audit, including sampled-geometry and global-time tradeoffs."""
import hashlib
import json
...
def main():
    root=Path('question3/results');lock=json.loads((root/'round9_lock.json').read_text())
    assert all(hashlib.sha256(Path(p).read_bytes()).hexdigest()==h for p,h in lock['code_sha256'].items())
    names=['round9_dev_main','round9_dev_rounding','round9_adverse_main','round9_adverse_rounding',
           'round9_holdout_main','round9_holdout_rounding','round9_outer_main','round9_outer_rounding']
    report=dict(selected_before_validation='ZL',batches={},code_lock_unchanged=True,
                official_runs=0,formal_runs=0,fresh_validation_runs=2800,fresh_layouts=70)
    summaries={};legacy=0
    for name in names:
    ...
    for suffix in ['main','rounding']:
        old=json.loads((root/f'round8_dev_{suffix}'/'summary.json').read_text())
        lookup={(r['variant'],r['case_id']):r for r in old['cases']}
        for r in summaries[f'round9_dev_{suffix}']['cases']:
            if r['variant'] in ('REF','J','Z'):
                for k in ['metrics','status','cleared','remaining_channels','audit']:assert r[k]==lookup[r['variant'],r['case_id']][k]
                legacy+=1
    gates=[]
    for name in names[4:]:
        stats={r['variant']:r for r in json.loads((root/name/'comparison.json').read_text())['stats']}
        a,b=stats['REF'],stats['ZL']
    ...
    (root/'round9_review_evidence.json').write_text(json.dumps(report,indent=2)+'\n')
