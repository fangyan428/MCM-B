...
def main():
    root=Path('question3/results');lock=json.loads((root/'round5_lock.json').read_text());selected=lock['selected_variant']
    assert all(hashlib.sha256(Path(p).read_bytes()).hexdigest()==h for p,h in lock['code_sha256'].items())
    names=['round5_dev_main','round5_dev_rounding','round5_structured_dev_main','round5_structured_dev_rounding','round5_holdout_main','round5_holdout_rounding','round5_outer_main','round5_outer_rounding']
    ...
    for name in names:
        ...
        pairs=[]
        for a,b in [('REF','REF_BATCH'),('COMPACT','COMPACT_BATCH'),('REF','COMPACT'),('REF_BATCH','COMPACT_BATCH'),('REF','COMPACT_BATCH'),('AB','COMPACT_BATCH')]:
    ...
    adoption=[]
    for name in names[4:]:
        comparison=json.loads((root/name/'comparison.json').read_text());s={r['variant']:r for r in comparison['stats']};candidate=s[selected];base=s['REF']
        adoption.append(dict(batch=name,pass_criteria=(candidate['complete']==candidate['runs'] and candidate['timeouts']==0 and candidate['false_complete']==0 and candidate['mean_virtual_s']<base['mean_virtual_s'] and candidate['p95_virtual_s']<=base['p95_virtual_s']),mean_reduction_fraction=1-candidate['mean_virtual_s']/base['mean_virtual_s']))
    ...
    result['official_rehearsals_run']=0;result['formal_tests_run']=0
    (root/'round5_review_evidence.json').write_text(json.dumps(result,indent=2));print(json.dumps({k:v for k,v in result.items() if k!='batches'},indent=2))
