...
def audit_folder(folder):
    ...
    position=(0.,0.);channel=1;vt=0.;done=set();bearings={};measure_moves={};checks=0
    for line in (folder/'actions.jsonl').read_text(encoding='utf-8').splitlines():
        ...
        if row['type']=='request':requests[row['payload']['request_id']]=row
            ...
            if path in ('/measure','/clear'):
                p=(b['position']['x'],b['position']['y']);c=b['channel'];move=math.dist(position,p);vt+=move/5;position=p
    ...
    assert np.allclose(strategy['station_positions'],stations(radius),atol=1e-7,rtol=0)
    ...
    for line in (folder/'strategy.jsonl').read_text(encoding='utf-8').splitlines():
        ...
        k=event['halfplane_index'];p=np.asarray(event['point']);gap=float((A[k]@p-b[k])/np.linalg.norm(A[k]))
