...
def main():
    root=Path('question3/results/round7_k_release_http');root.mkdir(exist_ok=False)
    config=Path('question3/configs/round7_k_holdout/K.json');rows=[]
    for case in [random_case(0),clustered_case()]:
        ...
        try:
            ...
            rows.append(dict(case_id=case['id'],stopping_certificate=result.get('stopping_certificate'),kind='SELF_REAL_LOOPBACK_HTTP_NOT_OFFICIAL',status=result['status'],virtual_time_s=result['metrics']['virtual_time_s']))
