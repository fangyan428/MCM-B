...
from question4.innovation.analysis import analyze
...
def source_hashes():
    ...
    paths.update((ROOT / 'frozen').rglob('*.py'))
    paths.update((ROOT / 'frozen').rglob('*.json'))
...
def _run_case(task):
    ...
    for name in names:
        ...
        try:
            from question4.innovation.audit import audit_regions
            checked['regions'] = audit_regions(case, events)
            from .checks import check_additional
            checked['additional'] = check_additional(case, records, result, events)
            from .optical_audit import check_cells
            checked['optical_cells'] = check_cells(case, events)
        ...
        row = dict(case_id=case['id'], layout_id=case['layout_id'], layout_sha256=case['layout_sha256'],
                   ...
                   timeout=bool(result.get('failure') and any(s in result['failure'].lower() for s in ('deadline', 'timeout', 'expired'))),
...
def main():
    ...
    if args.lock:
        ...
        if is_validation and (args.seed != previous['generator_seed'] or args.count != previous['generator_count'] or error_modes != previous['error_modes'] or roundings != previous['roundings']):
            parser.error('Validation plan differs from pre-generation lock')
    ...
    try:
        ...
        for name, stats in report['configurations'].items():
            stats['timeouts'] = sum(bool(r.get('timeout')) for r in rows if r['config']==name)
