...
def audit_folder(folder):
    ...
    for region in strategy['regions']:
        ...
        if selection.get('mode') in ('domain_cost_proxy','domain_current','domain_shared'):
            first=tuple(region['first_position']);second=tuple(region['second_position']);c=region['channel']
            theta=math.radians(bearings[first,c])%(2*math.pi);d=math.radians(selection['error_deg'])
            assert selection['error_deg']>=1.005 and selection['range_prior']=='published_domain_1800'
            assert 4*selection['error_deg']<selection['min_angle_deg']<90
            rho=math.hypot(*first);assert rho<=1800
            directions=[theta-d,theta+d];opposite=math.atan2(first[1],first[0])+math.pi
            directions.extend(opposite+2*k*math.pi for k in range(-2,3) if theta-d<=opposite+2*k*math.pi<=theta+d)
            projection=min(first[0]*math.cos(t)+first[1]*math.sin(t) for t in directions)
            bound=min(1500.,-projection+math.sqrt(max(0.,projection**2+1800**2-rho**2)))
            upper=selection['source_range_upper_m'];assert 5<upper<1500 and upper>=bound-1e-7
            g=(sources[c]['x'],sources[c]['y']);assert math.dist(g,first)<=upper+1e-7
            dx=second[0]-first[0];dy=second[1]-first[1]
            a=dx*math.cos(theta)+dy*math.sin(theta);b=-dx*math.sin(theta)+dy*math.cos(theta)
            assert np.allclose((a,b),selection['offset'],atol=1e-7,rtol=0) and a>=-1e-7
            h=a*math.cos(d)-abs(b)*math.sin(d);n=a*a+b*b
            radii=[5,upper]+([1000] if upper>1000 else [])
            for radius in radii:assert n+radius**2-2*radius*h<=max(1000,radius)**2+1e-6
            maximum=math.sqrt(max(n+25-10*h,n+upper**2-2*upper*h))
            cross=abs(b)*math.cos(d)-a*math.sin(d)
            assert cross>5 and cross>=maximum*math.sin(math.radians(selection['min_angle_deg']))-1e-7
            assert (second,c) in bearings
            if selection['mode'] in ('domain_current','domain_shared'):assert measure_moves[second,c]==0
