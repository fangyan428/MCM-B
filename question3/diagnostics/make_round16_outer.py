"""Evaluator-only fresh outer layouts, generated after the round-16 code/config lock."""
import hashlib
import json
import math
...
def main():
    root=Path('question3/results');lock=json.loads((root/'round16_lock.json').read_text())
    for p,h in lock['code_sha256'].items():
        if hashlib.sha256(Path(p).read_bytes()).hexdigest()!=h:raise RuntimeError('Lock changed: '+p)
    out=root/'round16_outer_holdout_cases.json'
