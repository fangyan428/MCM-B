...
def worthwhile(poly,bearing,point,error=1.005):
    """Conservative all-output contraction gate, NOT a sampled source probability."""
    old=bbox(poly,bearing)[1]
    if old<=20-1e-5:return False
    point=np.asarray(point)
    if contains(poly,point):
        low,high=0.,360.
    else:
        ref=math.degrees(math.atan2(*(poly.mean(axis=0)-point)[::-1]))
        angles=[ref+(math.degrees(math.atan2(*(v-point)[::-1]))-ref+180)%360-180 for v in poly]
        low,high=min(angles)-error,max(angles)+error
    width=(high-low)/16
    worst=0.
    for i in range(16):
        mid=low+(i+.5)*width
        try:
            posterior=wedge(poly,point,mid,error+width/2)
        except RuntimeError:
            continue
        worst=max(worst,bbox(posterior,mid)[1])
    return worst<=max(20-1e-5,old/2)-1e-5
