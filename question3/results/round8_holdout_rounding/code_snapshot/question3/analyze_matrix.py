...
def main():
    p=argparse.ArgumentParser();p.add_argument('folder');p.add_argument('--reference',default='baseline');args=p.parse_args();root=Path(args.folder)
    ...
    variants=list(data['configs'])
    if args.reference not in variants:raise ValueError('Missing reference variant')
    for name in variants:
        rows=[lookup[name,i] for i in ids];base=[lookup[args.reference,i] for i in ids]
    ...
    for a,b in [('A','B'),('A','C'),('B','C')]:
        if not all(k in mean for k in [a,b,a+b,'baseline']):continue
    ...
    report=dict(reference=args.reference,kind=data['kind'],rounding=data['rounding'],stats=stats,effects=effects,interactions=interactions,failures=failures,
                notes=['All runs retained. Failure/timeout/noncertified runs ranked with >=360000s penalty.',
                       'Percentiles include all cases; experiment kind identifies development versus held-out fixtures.'])
