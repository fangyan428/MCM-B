"""Exercise BOTH through the real CLI and local self HTTP; not official."""
import json
...
def main():
    root=Path('question3/results/round19_self_http');root.mkdir(exist_ok=False);rows=[]
    for case in [random_case(0),clustered_case()]:
        name=case['id']
        env=Simulator(case);server=make_server(env)
        thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start()
        out=root/name;config=Path('question3/configs/round19_dev','BOTH.json')
        try:
            ...
            count=sum(e.get('forced',False) for e in tasks)
            assert count==0 and len(tasks)<=23
            shared=sum(e['type']=='localization' and e['selection']['mode']=='domain_shared' for e in es)
            if name=='clustered_16':assert shared==15
            ...
            rows.append(dict(variant=name,kind='SELF_REAL_LOOPBACK_HTTP_NOT_OFFICIAL',complete=True,
                forced_scans=count,scheduled_tasks=len(tasks),domain_shared=shared,virtual_time_s=result['metrics']['virtual_time_s']))
