...
算法输出实际任务访问序列、各频道清除记录、式（2）的时间分解及停止依据。25站布局、重心排序与观测筛选共同组成第四问推荐方案，其自建配对实验和其他布局、动态测站的比较见6.3节，正式测试见6.4节；不能将完整方案的降时比例归为站数变化的单独贡献。各问模块的输入、输出与适用条件汇总于附录A.3。
...
本章主要性能结果与统计图均由已冻结的逐例记录计算。Q2是自建几何算例；Q3和Q4为按题面及接口规则实现的自建完整流程实验。自建源布局用于比较策略，不被解释为官方案例的概率分布。几何复核、实验记录与正式测试材料的说明见附录B、C及随文支撑材料。后续新增的Q3调度开发比较单列于附录B.5，不并入原验证统计。
...
实现核验覆盖空集、无界与退化区域，以及公开响应重放、不同源清除计数和停止条件；Q2的专用几何计算另与通用半平面求解器交叉比较。检查支持实现与既有记录的一致性，不能替代实数几何证明或浮点程序的形式验证。具体测试规模见附录B.6。
...
另外，单方位光学覆盖与沿接收边界测距都增加了开发阶段的整局耗时。它们说明``局部可行''不足以成为常规动作的选择依据，具体开发对照保留在附录B.4。
...
早期开发已发现，先搜索后清除等固定阶段安排会失去顺路服务机会。动态替站与测站移动组合虽在开发中省时2.33\%，仍未达到事前5\%的替换门槛，因此后续锁定验证检验的是收益能否迁移，而不是继续寻找有利参数。开发细节见附录B.4。
...
图21从发现、定位和清除三个环节补充解释整局表现。上排对应第6.3.1节的Q4历史配对实验：按原始汇总，首次发现全部源所需扫描站数的中位数，在读数包络批次由20降至15，在舍入压力批次由20.5降至18。这支持25站推荐在这些布局上较早积累发现信息，但不表示每个布局均改善；首次全发现是依据隐藏真值计算的事后指标，不足16源时客户端仍需完成停止核验。
...
\label{fig:process-evidence}
\end{figure}
中排对应Q3三次正式运行的38条源轨迹。原始区域记录中，31个源最终满足覆盖圆清除判据，另7个源通过近场反馈清除，因此曲线在20 m以上结束不等于定位失败。下排将成功清除数与累计虚拟时间联系起来：相邻清除点之间的耗时包括其间的搜索、补测、移动与光学尝试，不能单独归给一次清除动作。原始记录还显示，Q3正式3及Q4正式1、2在最后清除后分别继续核验140.85 s、463.79 s和123.77 s，说明清除终点与任务退出必须分开计量。
...
针对第三问，补充开发比较已尝试将未知区域覆盖收益、源服务费用及服务结束位置纳入排序，但四种候选均未达到预设选择条件，因而仍保留本文的联合方案，具体结果见附录B.5。这说明增加代价项并不必然改善整局效率；后续方案应在新布局上同时检验平均耗时和较慢案例的表现。现有自建布局也不足以确定官方源分布，不能直接将其中的出现频率当作可靠的发现概率。
