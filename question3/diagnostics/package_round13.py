"""Immutable public-radius BOTH runtime and compact experiment evidence; retain prior packages."""
import hashlib
import json
...
def main():
    r=Path('question3');out=r/'releases/q3_omni_round13.zip'
    if out.exists():raise RuntimeError('Do not overwrite release')
    lock=json.loads((r/'results/round13_lock.json').read_text())
    ...
    with ZipFile(r/'releases/q3_omni_round11.zip') as z:files={Path(p) for p in z.namelist()}
    ...
    files.update([r/'STATUS.md',r/'README.md',r/'RUN_RECOMMENDED.md',r/'releases/recommended_manifest.json',r/'releases/recommended_manifest_round11.json'])
    for prefix in ['round12','round13']:
        files.update(p for p in (r/'results').glob(prefix+'_*') if p.is_file() and p.name not in ('round13_package_build.json','round13_release_package_smoke.json'))
    ...
    for b,c,variants in [('round13_adverse_main','outer_holdout_120008_spatial_hash',['REF','BOTH']),
                         ('round13_holdout_main','holdout_170023_spatial_hash',['REF','BOTH']),
                         ('round13_dev_main','dev_0_negative',['REF','DOMAIN','RANGE','BOTH']),
                         ('round13_dev_main','dev_1_smooth',['REF','BOTH'])]:
        for v in variants:files.update((r/'results'/b/v/c).glob('*.*'))
    for d in ['round13_self_http']:
