...
def main():
    root=Path('question3/results/round13_self_http');root.mkdir(exist_ok=False);rows=[]
    for name in ['BOTH']:
        env=Simulator(random_case(0,'negative'));server=make_server(env)
        thread=threading.Thread(target=server.serve_forever,daemon=True);thread.start()
        out=root/name;config=Path('question3/configs/round13_dev',name+'.json')
        try:
            ...
            count=sum(e['type']=='certified_clear_skip' and 'radius_prior' in e for e in es)
            assert count>0
            ...
            rows.append(dict(variant=name,kind='SELF_REAL_LOOPBACK_HTTP_NOT_OFFICIAL',complete=True,
                radius_prior_skips=count,virtual_time_s=result['metrics']['virtual_time_s']))
