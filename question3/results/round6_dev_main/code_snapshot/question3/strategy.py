...
def run(client, config, event=lambda data:None):
    ...
    def localize(channel,q,selection):
        if channel in cached:
            ...
            for _ in range(shared_limit if shared else 0):
    ...
    try:
        while True:
            ...
            decision=decide_order(*args,cached=cached,forced=forced) if order else decide(*args,forced=forced)
            if cache_route=='station_insertion':
                decision=reorder_cached(decision,client.position,cover,remaining,jobs,cached)
