...
import numpy as np
from question4.geometry import segment_distance
...
def audit(case,records,result):
    ...
    if cert['kind']=='count16':
        ...
        if cert['kind']=='dynamic_cell_halfplane_cover':
            assert np.array_equal(points,check_cover(json.dumps(cert['dynamic_cover'],sort_keys=True)))
        ...
        assert contains_origin or cert['kind'] in ('cell_halfplane_cover','dynamic_cell_halfplane_cover')
