"""SELF simulation of mixed omnidirectional/directional physics and valid-action protocol.
    ...
    def request(self,path,payload,timeout=5.):
        ...
        if path=='/enter':
            ...
            if path=='/measure':
                ...
                visible = axis is None or ((p[0]-s['x'])*math.cos(math.radians(axis))+(p[1]-s['y'])*math.sin(math.radians(axis)) >= -1e-10)
                if s is None or distance>s['radius'] or not visible:result=dict(measure_result='no_signal')
